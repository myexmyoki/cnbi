...
class HolidaysRequest(models.Model):
    ...
    payslip_status = fields.Boolean('Reported in last payslips', help='Green this button when the Leave has been taken into account in the payslip.', copy=False)
    ...
    number_of_days = fields.Float(
        ...
        help='Number of days of the Leave request. Used in the calculation. To manually correct the duration, use this field.')
        ...
        for holiday in self:
            ...
            if float_compare(leave_days['remaining_leaves'], 0, precision_digits=2) == -1 or float_compare(leave_days['virtual_remaining_leaves'], 0, precision_digits=2) == -1:
                raise ValidationError(_('The number of remaining Leave is not sufficient for this Leave type.\n'
                                        'Please also check the Leave waiting for validation.'))
    ...
    def create(self, values):
        ...
        if not self._context.get('leave_fast_create'):
            ...
            if leave_type.validation_type == 'no_validation':
                ...
                holiday_sudo.message_post(body=_("The Leave has been automatically approved"), subtype="mt_comment") # Message from OdooBot (sudo)
    ...
    def _prepare_holidays_meeting_values(self):
        ...
        if self.leave_type_request_unit == 'hour':
            meeting_name = _("%s on Leave : %.2f hour(s)") % (self.employee_id.name or self.category_id.name, self.number_of_hours_display)
        else:
            meeting_name = _("%s on Leave : %.2f day(s)") % (self.employee_id.name or self.category_id.name, self.number_of_days)
        meeting_values = {
            'name': meeting_name,
        ...
        for holiday in self:
            if holiday.employee_id.user_id:
                holiday.message_post(
                    body=_('Your %s planned on %s has been declined') % (holiday.holiday_status_id.display_name, holiday.date_from),
    ...
    def action_approve(self):
        ...
        for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
            holiday.message_post(
                body=_('Your %s planned on %s has been endorsed by your supervisor. Please Wait for final approval decision by the HRM. You will be notified on the outcome in due course' % (holiday.holiday_status_id.display_name, holiday.date_from)),
                partner_ids=holiday.employee_id.user_id.partner_id.ids)
    ...
    def action_validate(self):
        ...
        #Test HRM Appoval Email
        for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
            holiday.message_post(
                body=_('Your %s planned on %s has been approved by the HRM. Please plan accordingly' % (holiday.holiday_status_id.display_name, holiday.date_from)),
                partner_ids=holiday.employee_id.user_id.partner_id.ids)
    ...
    def action_refuse(self):
        ...
        if any(holiday.state not in ['confirm', 'validate', 'validate1'] for holiday in self):
            raise UserError(_('Leave request must be confirmed or validated in order to Decline it.'))
    ...
    def _notify_get_groups(self):
        ...
        if self.state in ['confirm', 'validate', 'validate1']:
            ...
            hr_actions += [{'url': ref_action, 'title': _('Decline')}]
