...
class HolidaysAllocation(models.Model):
    ...
    first_approver_id = fields.Many2one(
        'hr.employee', string='Awaiting Supervisor Approval', readonly=True, copy=False,
        help='This area is automatically filled by the user who validate the allocation')
    second_approver_id = fields.Many2one(
        'hr.employee', string='Awaiting HRM Approval', readonly=True, copy=False,
    ...
    def action_confirm(self):
        ...
        for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
            holiday.message_post(
                body=_('Your leave allocation request has been submitted successfully.'),
    ...
    def action_approve(self):
        ...
        for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
            holiday.message_post(
                body=_('Your subject request has been endorsed by your supervisor'),
                partner_ids=holiday.employee_id.user_id.partner_id.ids)
              #MAX ADDED END  
    ...
    def action_validate(self):
        ...
        for holiday in self:
            ...
            if holiday.validation_type == 'both':
                ...
                for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
                    holiday.message_post(
                        body=_('Your subject request has been endorsed approved by the HRM'),
                        partner_ids=holiday.employee_id.user_id.partner_id.ids)
                #MAX ADDED END 
            else:
    ...
    def action_refuse(self):
        ...
        for holiday in self.filtered(lambda holiday: holiday.employee_id.user_id):
            holiday.message_post(
                body=_('Your subject request has been declined'),
                partner_ids=holiday.employee_id.user_id.partner_id.ids)
        #MAX ADDED END
        ...
        for holiday in self:
            ...
            if state == 'draft':
                if holiday.employee_id != current_employee and not is_manager:
                    raise UserError(_('Only a Leave Manager can reset other people allocation.'))
                continue

            if not is_officer and self.env.user != holiday.employee_id.leave_manager_id:
                raise UserError(_('Only the HRM /Responsible or Manager can approve or refuse Leave requests.'))
            ...
            if holiday.employee_id == current_employee and not is_manager:
                raise UserError(_('Only a HR Manager can approve its own requests.'))
            ...
            if (state == 'validate1' and val_type == 'both') or (state == 'validate' and val_type == 'manager'):
                ...
                if (manager and manager != current_employee) and not self.env.user.has_group('hr_holidays.group_hr_holidays_manager'):
                    raise UserError(_('You must be either %s\'s manager or HR manager to approve this Leave') % (holiday.employee_id.name))

            if state == 'validate' and val_type == 'both':
                if not self.env.user.has_group('hr_holidays.group_hr_holidays_manager'):
                    raise UserError(_('Only the HR Manager can do final approval of leave  allocation requests.'))
